...
module.exports = new Event("guildMemberRemove", async (client, member, interaction, guild) => {
    ...
    if (supportbot.Leave.Enabled) {
        const LeaveChannel = member.guild.channels.cache.get(supportbot.Leave.Channel) ||
                             member.guild.channels.cache.find(channel => channel.name === supportbot.Leave.Channel);

        const LeaveEmbed = new Discord.EmbedBuilder()
            .setColor(msgconfig.Leave.Embed.Colour)
            .setTitle(msgconfig.Leave.Embed.Title)
            .setDescription(msgconfig.Leave.Embed.Message.replace(/%left_user%/g, member.user))
            .setTimestamp();
